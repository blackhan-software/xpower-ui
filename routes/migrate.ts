...
router.get('/', (req, res) => {
  ...
  const { THOR_SOV_IMAGE_V5a } = env;
  assert(THOR_SOV_IMAGE_V5a, 'missing THOR_SOV_IMAGE_V5a');
  const { LOKI_SOV_IMAGE_V5a } = env;
  assert(LOKI_SOV_IMAGE_V5a, 'missing LOKI_SOV_IMAGE_V5a');
  const { ODIN_SOV_IMAGE_V5a } = env;
  assert(ODIN_SOV_IMAGE_V5a, 'missing ODIN_SOV_IMAGE_V5a');
  res.render('migrate/index.pig', {
    ...
    THOR_SOV_IMAGE: THOR_SOV_IMAGE_V5a,
    LOKI_SOV_IMAGE: LOKI_SOV_IMAGE_V5a,
    ODIN_SOV_IMAGE: ODIN_SOV_IMAGE_V5a,
  });
});
