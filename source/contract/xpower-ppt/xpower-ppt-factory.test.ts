...
import { XPowerPptMockFactory } from './xpower-ppt-factory';
...
describe('XPowerPptMockFactory', () => {
    it('should return totalSupply(id=1202100) = 0', async () => {
        const nft = await XPowerPptMockFactory({ token: Token.THOR });
...
describe('XPowerPptMockFactory', () => {
    ...
    it(`should fetch & validate uri(id=1${year}00)`, async () => {
        const nft = await XPowerPptMockFactory({ token: Token.THOR });
        const uri = nft.uri(nft.idBy(nft.year(), 0, 0)); // current!
        ...
        await request(gateway).get(uri)
            ...
            .expect(({ body: json }) => {
                ...
                expect(json.image).toMatch(new RegExp(`${nft.year()}/Thor/1-Thor_UNIT.png$`))
                expect(json.attributes).toBeDefined();
            });
    });
    it(`should fetch & validate uri(id=2${year}03)`, async () => {
        const nft = await XPowerPptMockFactory({ token: Token.LOKI });
        const uri = nft.uri(nft.idBy(nft.year(), 3, 1)); // current!
        expect(uri).toBeDefined();
        const gateway = ipfs_gateway();
        expect(gateway).toBeDefined();
        await request(gateway).get(uri)
            ...
            .expect(({ body: json }) => {
                ...
                expect(json.image).toMatch(new RegExp(`${nft.year()}/Loki/2-Loki_KILO.png$`))
                expect(json.attributes).toBeDefined();
            });
    });
    it(`should fetch & validate uri(id=3${year}06)`, async () => {
        const nft = await XPowerPptMockFactory({ token: Token.ODIN });
        ...
        await request(gateway).get(uri)
            ...
            .expect(({ body: json }) => {
                ...
                expect(json.image).toMatch(new RegExp(`${nft.year()}/Odin/3-Odin_MEGA.png$`))
                expect(json.attributes).toBeDefined();
            });
    });
    it(`should fetch & validate uri(id=4${year}09)`, async () => {
        const nft = await XPowerPptMockFactory({ token: Token.HELA });
